...
- **Don't mark functions as async unnecessarily** - async functions use a less efficient calling convention than
  synchronous functions due to potential suspension points
- **Avoid excessive context switching** - minimise the number of `await` calls by grouping operations within the same
  isolation domain
...
**Important**: `@preconcurrency` doesn't make code thread-safe - it only relaxes compiler checks for backward
compatibility.
