...
public class Result<T> {
    ...
    // 成功响应（有数据，带路径）
    public static <T> Result<T> successWithDataAndPath(T data, String path) {
    ...
    // 成功响应（自定义消息，无数据，带路径）
    public static <T> Result<T> successWithMessageAndPath(String message, String path) {
        return new Result<>(200, message, null, path);
    }

    // 成功响应（自定义消息和数据）
    public static <T> Result<T> successWithMessageAndData(String message, T data) {
