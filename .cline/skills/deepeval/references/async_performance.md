...
DeepEval provides built-in async execution and performance optimisation features for evaluating LLMs at scale. This
guide covers practical implementation patterns, configuration options, and performance considerations.
...
**Explanation**: `async_mode=True` enables **internal** concurrent operations, but `measure()` is synchronous. Use
`a_measure()` for non-blocking execution.
